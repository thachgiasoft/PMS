                ...
                foreach (KhoiLuongGiangDayChiTiet klGiangDay in listKhoiLuong)
                {
                    if (!backgroundWorker.CancellationPending)
                    {
                        ...
                        else //if (klQuyDoi.MaGiangVien.Equals("011.208.00005")) //<< test //Thực hành
                        {
                            ...
                            //Các môn GDTC, XHNV tính theo số tín chỉ * 15
                            if ("TD;CP".Contains(klQuyDoi.MaKhoa))
                            {
                                klQuyDoi.TietQuyDoi = klQuyDoi.SoTinChi * 15;
                            }
                            //Các môn còn lại mà không thuộc khoa CNTT, TT thì = sĩ số / 25 * số tiết * 0.5
                            else if(!"CT;TN".Contains(klQuyDoi.MaKhoa))
                            {
                                _siSoNhomThucHanh = 25;
                                klQuyDoi.HeSoQuyDoiThucHanhSangLyThuyet = (decimal)0.5;
                            }
                            //Các môn vừa LT vừa TH của khoa tự nhiên hoặc môn thực tập phòng máy của CNTT,TT
                            else if ("CT;TN".Contains(klQuyDoi.MaKhoa))
                            {
                                _siSoNhomThucHanh = 15;
                                klQuyDoi.HeSoQuyDoiThucHanhSangLyThuyet = (decimal)0.5;
                                if (klQuyDoi.SoLuong < 15) klQuyDoi.SoLuong = 15;
                            }
                            ...
                            klQuyDoi.TietQuyDoi = (klQuyDoi.HeSoQuyDoiThucHanhSangLyThuyet * _soNhomThucHanh * klQuyDoi.SoTiet)
